const employees = [
    ...
    { name: "Виталя", greeting: "С днем рождения Лена! В этот особенный день хочу поздравить тебя и выразить свои искренние пожелания. Ты — не только замечательный руководитель, но и удивительный человек, который вдохновляет нас своим примером и поддержкой. Желаю тебе, чтобы каждый день приносил радость и счастье, чтобы в жизни было как можно больше светлых моментов и приятных сюрпризов. Пусть все мечты сбываются, а рядом всегда будут верные друзья и близкие люди. Спасибо за твою мудрость и терпение, за тот позитивный настрой, который ты даришь команде. Пусть впереди будет много новых достижений и ярких впечатлений!", character: 12, long: true },
